<template>
    <div>
        ...
        <div class="tableWp marginTop2">
            <el-table :data="tableData" size="small" stripe>
                ...
                <el-table-column label="操作" width="220px" header-align="center" align="center">
                    <template slot-scope="scope">
                        <el-button class="cyanTableBtn" size="mini" round v-if="scope.row.status != '2' "
        ...
        <el-dialog
                ...
                <el-row :gutter="20">
                    ...
                    <el-col :span="12">
                        <div v-if="show" class="class">
                            <span class="titletext">你所选择的收费站具有代收功能，以下为其代收门架的编号:</span>
                            <p> {{showtext}}</p>
                            <p class="titletext">注意：若代收关系有误，请联系运营人员维护。</p>
            ...
            <div slot="footer">
                <el-button class="redTableBtn" size="medium" round @click="resetFlag">取消</el-button>
                <el-button v-if="flag == 1" class="blueTableBtn" size="medium" round @click="amendDatadeploy">保存
                </el-button>
            </div>
        </el-dialog>
        <!--    详情-->
...
<script>
    ...
    export default {
        ...
        data() {
            return {
                roleName: this.$cookie.get('roleName') == '超级管理员' ? true : false,
        ...
        methods: {
            release(row) {
                ...
                this.$confirm('你确定释放此设备吗?', '提示', {
                    ...
                    var filename = api.BIND_RELEASES + getDataTime() + '.json';
            ...
            details(row) {
                ...
                _t.$api.post('api/json', data, function (res) {
                    ...
                    var status = '';
                    var value = _t.detailsList.deviceStatus
                    if (value == '0') {
                        status = '初始化'
                    }else if(value == '1'){
                        status = '从未部署'
            ...
            amendDatadeploy() {
                ...
                _t.$refs.deployFlag.validate((valid) => {
                    if (valid) {
                        ...
                        var filename = api.BIND_ADD + getDataTime() + '.json';
                        var data = this.changeData(params, filename, _t.$cookie.get('accessToken'));
                        _t.$api.post('api/json', data, function (res) {
                            if (res.statusCode == 0) {
                                _t.alertMessageTip(_t, res.errorMsg);
            ...
            redeploy(row) {
                ...
                _t.$confirm('你确定停用此设备吗?', '提示', {
                    ...
                    var filename = api.BIND_DELETE + getDataTime() + '.json';
                    var data = this.changeData(params, filename, _t.$cookie.get('accessToken'));
                    _t.$api.post('api/json', data, function (res) {
                        if (res.statusCode == 0) {
                            _t.refreshHandle();
                ...
                _t.$refs.addList.validate((valid) => {
                    if (valid) {
                        ...
                        if (_t.addEdit.deviceId.indexOf("@") != -1) {
                            ...
                            var filename = api.BIND_CONFIRM + getDataTime() + '.json';
                            var data = this.changeData(params, filename, _t.$cookie.get('accessToken'));
                            _t.$api.post('api/json', data, function (res) {
                                if (res.statusCode == 0) {
                                    _t.addVisible = false;
